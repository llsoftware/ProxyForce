...
cooperating. ProxyForce also neutralizes the Windows system proxy while it runs
(so proxy-aware apps can't slip straight past it) and suppresses IPv6 (AAAA) so
...
- **The Windows system proxy is auto-neutralized while ProxyForce runs** (both the
  per-user WinINET setting *and* machine-wide WinHTTP), then restored exactly as it
  was on stop. Without this, proxy-aware apps would send straight to the proxy and
  never enter the TUN. The original is snapshotted to
...
- **UDP is rejected** (including QUIC/HTTP3 on UDP/443) so apps fall back to TCP,
  which is captured. DNS is the one exception — it's hijacked to fakeip.
...
> **Your Windows proxy setting while running:** ProxyForce temporarily turns **off**
> the Windows system proxy (Settings ▸ Network ▸ Proxy, and `netsh winhttp`) so no
> app can bypass capture. This is expected — it's automatically restored to your
> exact previous setting when you Stop or Quit (and recovered from a backup file
...
> → system proxy neutralized), ending in a one-line **VERDICT**. The Log tab also
...
ProxyForce disables the Windows system proxy on Start, but a corporate **Group
Policy** can re-push it. Check the **"System proxy"** section of `diagnostics.txt`:
a WARN there means a GPO or a per-app proxy re-enabled it. GPO-enforced proxies must
be cleared by policy (or the app's own proxy setting changed); the TUN still
